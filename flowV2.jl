...
function calculateP(currP, xcord, ycord, prevX, prevY, currV, currU, deltaZ, P0)
	...
	if (deltaR == 0)
		deltaR = .01 #prevent divison by zero
	end
	...
	if(isnan(P))
		println("discovered P is NaN")
		println(string("delta Z is ", deltaZ, "currU is ", currU, "currP is", currP, " currX is ", xcord, "currY is", ycord))
		println(string("tauRR is", tauRR, "tauZZ is ", tauZZ, "tarRZ is ", tauRZ))
		println(string(("currR-prevR", (currR-prevR))))
		println(string("(currV*1/deltaR*currV+currU*1/deltaZ*currV)", (currV*1/deltaR*currV+currU*1/deltaZ*currV)))
		println(string("(1/currR*1/deltaR*currR*tauRR+1/deltaZ*tauRZ)"), (1/currR*1/deltaR*currR*tauRR+1/deltaZ*tauRZ))
		println(string("rho*1/deltaZ*currV"), rho*1/deltaZ*currV)
		#readline(STDIN)
		P = P0
	end
	return P
				...
				for n = 1:size(udata, 1)-1
					...
					for m = 1:size(udata,2)-1
						...
						if(n == desiredIndex)
							...
							uYZ[yindex, zindex] = udata[n,m]
							vYZ[yindex, zindex] = vdata[n,m]
							yindex = yindex +1
							#println("uYZ is")
							#println(uYZ)
						end
...
function main()
	...
	R0 = 1 #initial radius of blood vessel, in cm
	#from http://classes.midlandstech.edu/carterp/Courses/bio211/chap19/chap19.html for elastic artery
	Rwall = R0;

	t0 = 0.0
	tsim = t0
	zSim = 0.0
	P0 = 1.333 #inital pressure, in N/cm^2
	#from Continusous vs Intermittent Blood Pressure Measurements in Estimating 24-hour average blood pressure
		...
		while zSim < zEnd
			...
			#not moving through coordinates. Need to figure out why
					...
					if(inVessel == 1)
						...
						if(xindex > 1)
							P[xindex-1, yindex]= calculateP(currP, xcord, ycord, prevX, prevY, currV, currU, deltaZ,P0)
