...
function complexHeartModel(t,y,dydt,data_dict)
	...
	@show fabmin, fabmax, Psquiggle, Pn, kab
	...
	@show fesinf, fes0, kes, Wbsh, fab, Wbsh, fac, Thetash, fesmax
	if(fes < fesmax)
		fsh = fesinf +(fes0-fesinf)*exp(kes*(-Wbsh*fab+Wcsh*fac-Thetash))
	elseif(fsh>=fesmax)
		fsh = fesmax
...
#	end
	...
	dPbdt = dPsadt
	@show taupb, Pb, tauzb, dPbdt, Psquiggle
...
function main()
	...
	prob = ODEProblem(fedeqns, initial_conditions,t)
	sol = solve(prob,Vern9(), dt = .02, abstol = 1E-2, reltol = 1E-2)
	toc()
	#psi = [a[14] for a in res]
	#@show res
	#psi = res[:, 14]
	#plot(tout, mod(psi,1), "kx")
	plotEverything(sol.t, sol, data_dict, "figures/EverythingStep.1absrel1E-1AttemptToRecreateFig13Everything.pdf")
#	plotPretty(t, sol, data_dict, "figures/PrettyStep.1absrel1E-1200sNoBleeAttemptToRecreateFig13Pretty.pdf")
