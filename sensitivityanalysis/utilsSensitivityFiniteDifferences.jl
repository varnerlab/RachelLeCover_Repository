...
require("OlufsenModel2011calcHRSensitivites.jl")
...
@everywhere function processNumericalData(filename)
	try
		...
		writetable("tempoutputSDa.csv", df)
		dfnew = readtable("tempoutputSDa.csv", separator=',')
		return dfnew, units
	catch
		Exception
...
@everywhere function calculateS0extended(DFhr,DFdxdp,changableparams)
	tic()
	S0 = Float64[]
	@show DFhr
	@show DFdxdp
	numparams = 5
	for j in collect(1:numparams)
		sum = 0.0
		for i in range(1, size(DFdxdp,1))
			@show 1/DFhr[i,1][1]
			@show DFdxdp[i,j][1]
...
function parallel_doSensitivitiesMakeDFs(params, setnumber)
	 N = 75.0
        M = 120.0
	 beta = 6
	#params= [75,1.5,.5,250, .5, .5, 1.67,.96, .7]
	...
	outputdir = "ParallelUsingFDSlowerCoolingParamsCluster1n10Jun22/"
	@show outputdir
	#touch(string(outputdir, "AllS0.txt"))
	#allpatients = readdir(inputdir)
	...
	allres= @parallel (vcat) for patient in allpatients
		...
		if(!in(:_Elapsed_time_, colnames))
				...
				if(size(data,1)==0)
					...
					for row in eachrow(data)
						...
						if(contains(string(typeof(res)), "Void"))
							...
							push!(DFdxdp, S)
							push!(DFhr, hr)
							f = open(string(outputdir, "setnumber", setnumber,"SensitivitiesForPatient", patient, ".csv"), "a")
							write(f, string(currt, ",", S, "\n"))
							close(f)
							rowcounter = rowcounter+1
						end
			...
			if(patientOfInterest==true)
				S0=calculateS0(DFhr,DFdxdp,params)
				g = open(string(outputdir, myid(), "set", setnumber,"AllS0.txt"), "a")
				write(g, string(patient, ",", S0, "\n"))
				close(g)
			end
				end
...
function doExtendedSensitivitiesMakeDFs_parallel()
	 N = 75.0
        M = 120.0
	 beta = 6
	#params= [75,1.5,.5,250, .5, .5, 1.67,.96, .7, 1.5, 6, 120, 1.5, 7]
	#params = [77.21810611260562,14.0659798715,2.3595665995,252.9553108258,0.4708140794,9.7720706413,1.7454283859,0.1612547652,0.30946585943989047]
	params =[107.33612908104033,41.138842557517954,14.062116217939991,202.92519160829795,2.941876046317941e-5,8.15596008945017,1.4743606995666585,0.1654851281383664,0.05218327316554491,1.5, 6, 120, 1.5, 7]
	inputdir = "/home/rachel/Documents/modelingHR/LinkedRecordsTimeData10min/"
	#outputdir = "/home/rachel/Documents/optimization/sensitivityanalysis/usingBestParamsJun20/"
	outputdir = "/home/rachel/Documents/optimization/sensitivityanalysis/usingBestParamsJun27ExtendedSensitivities/"
	touch(string(outputdir, "AllS0.txt"))
	cluster1path = "/home/rachel/Documents/optimization/multiobjective/usingPOETs/cluster1subjectIDs"
	cluster2path = "/home/rachel/Documents/optimization/multiobjective/usingPOETs/cluster2subjectIDs"
	...
	allres= @parallel (vcat) for patient in allpatients
		...
		if(!in(:_Elapsed_time_, colnames))
			...
			sort!(data, cols = [order(:_Elapsed_time_)])
			DFdxdp = DataFrame(dhdalpha = Float64[], dhdbeta = Float64[], dhdM = Float64[], dhdk2 = Float64[], dhdtaud = Float64[])
			DFhr = DataFrame(HR = Float64[])
				...
				if(size(data,1)==0)
					...
					for row in eachrow(data)
						...
						if(contains(string(typeof(res)), "Void"))
							...
							pbar = res[9]
							phi = res[10]
							S= calculateSensitivitesUsingCDForSetParams(cnor, cach,fsym, fpar,n,currt,params,Pdata,tdata,nsprev,bfprev)
							push!(DFdxdp, S)
							push!(DFhr, hr)
							f = open(string(outputdir,"SensitivitiesForPatient", patient, ".csv"), "a")
							write(f, string(currt, ",", S, "\n"))
			...
			if(patientOfInterest==true)
				S0=calculateS0extended(DFhr,DFdxdp,params)
...
function MultiObjdoExtendedSensitivitiesMakeDFs_parallel(params,setnumber)
	 N = 75.0
        M = 120.0
	 beta = 6.0
	alpha = 1.5
