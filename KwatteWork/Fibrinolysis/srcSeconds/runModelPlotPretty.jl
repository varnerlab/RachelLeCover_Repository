...
function runModelPlotPretty()
	...
	for j in collect(8:8:8*18)
		plt[:subplot](18,8,j)
		annotate(speciesnames[counter], xy = [1;1], xytext = [1.02,.8], xycoords = "axes fraction", textcoords = "axes fraction", rotation = "vertical", fontsize = 10)
		counter=counter+1
	end
	savefig(string("testingPrettyLayout",".pdf"),bbox_inches="tight")
end
