...
function generateData(num_param_sets, patientID)
	...
	for j in collect(1:num_param_sets)
		...
		outputfilename = string("outputOct6/600s/","for_patient", patientID,"set",j, ".txt")
...
function plotMeanAllCompartments(dir, num_param_sets, patientID)
	...
	for j in collect(8:8:8*18)
		plt[:subplot](18,8,j)
		annotate(speciesnames[counter], xy = [1;1], xytext = [1.02,.8], xycoords = "axes fraction", textcoords = "axes fraction", rotation = "vertical", fontsize = 5)
		counter=counter+1
	end
	...
	savefig(string("outputOct6/600s/patient", patientID, "AllCompartments.pdf"))
end
