include("runModelPeturbICAndV.jl")

function generateDiffVolsData(inputstr,n)
  for j in collect(1:n)
    savestr = string(inputstr, j)
    runModel(savestr)
  end
end

function plotcomparison(inputstr, n)
	close("all")
  ...
  colorstr = ".9"
	...
	savefig("DiffereingInitialConcentrationsandVolsDiffConctraction.pdf")
end

function plotMean(inputstr, n)
	close("all")
	#constP = readdlm("outputs/UsingConstP.csv")
	#varryingP = readdlm("outputs/s01004-2531-07-20-08-50n.csv")
...
function plotMeanOtherDir(inputstr, n)
	close("all")
	#constP = readdlm("outputs/UsingConstP.csv")
	#varryingP = readdlm("outputs/s01004-2531-07-20-08-50n.csv")
	...
	for k in collect(57:64)
		...
		plt[:tick_params](axis="both", which="minor", labelsize=14)
		#plt[:ticklabel_format](axis="y", useOffset=false)
		postive95conf = means[k]+1.96.*stdevs[k]
		negative95conf = means[k]-1.96.*stdevs[k]
		#  @show means[currindex]
		plot(t,means[k] ,linewidth=2.0,color = "k")
		fill_between((t), squeeze(negative95conf,2), squeeze(postive95conf,2), color = ".75")
		ax = gca()
	...
	plt[:subplot](8,8,64)
	ax = gca()
	#PyPlot.locator_params(axis="x",nbins=8 ) #reduce number of ticks
	ax[:set_xticklabels]([collect(0:5:45)])
	...
	savefig("output/DifferingInitialVolumesAndConditionsPrettyDiffContraction.pdf",bbox_inches="tight")
	  #return forsummary
...
function generateAndPlot()
  inputstr = "output/perturbICAndVDiffContraction"
  generateDiffVolsData(inputstr,100)
  plotcomparison(inputstr,100)
  plotMeanOtherDir(inputstr, 100)
end
