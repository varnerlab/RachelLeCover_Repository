...
function main(xml_filename, desired_species_filename,output_file_name)
	...
	generate_plotting_code(to_plot, output_file_name, compound_dict,0,10,.02)
...
function writePlotting(openfile, to_plot)
	...
	if(numPlots > 5)
		...
		while(counter<=numPlots)
			...
			write(openfile, string("\t plot(t, ", to_plot[counter], ")\n"))
			write(openfile, string("\t title(\"", to_plot[counter], "\")\n"))
			counter = counter+1;
		end
	else
		while(counter <= numPlots)
...
function generate_plotting_code(to_plot, output_file_name, compound_dict, tstart, tend, step)
	...
	f = writePlotting(f, to_plot)

	write(f, "\t end\n")
	write(f, "runModel()\n")
	close(f)
end
#to do: get user list of species to graph
#create code to do graphing

main("debug/PBPK_AST.xml", "debug/desired_species.txt", "src/runModel.jl")
