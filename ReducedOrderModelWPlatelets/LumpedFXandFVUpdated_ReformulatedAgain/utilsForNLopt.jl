...
function objectiveForNLOpt(params::Vector, grad::Vector)
	...
	t,X = ODE.ode23s(fbalances,(initial_condition_vector),TSIM, abstol = 1E-3, reltol = 1E-4)
	FIIa = [a[2] for a in X]
	MSE, interpolatedExperimentalData=calculateMSE(t, FIIa, experimentaldata)
	#hold("on")
	#plot(t, FIIa, alpha = .5)
	#write params to file
	f = open("parameterEstimation/NLopt_2017_02_07.txt", "a+")
...
function attemptOptimizationNLOpt()
	...
	upperbounds[17] = 50.0 #bound k_amp_active_factors to be small
	upperbounds[19] = 5.0 #bound k_PT_deg to be small
   ...
   kinetic_parameter_vector = Float64[]
    push!(kinetic_parameter_vector,7200/25)       #  k_trigger
    push!(kinetic_parameter_vector,10)       # 1 K_trigger
    push!(kinetic_parameter_vector,4.5)        # 2 k_amplification
    push!(kinetic_parameter_vector,120)       # 3 K_FII_amplification
    push!(kinetic_parameter_vector,0.1*100)        # 4 k_APC_formation
    ...
    push!(kinetic_parameter_vector,0.001*1.3)     # 8 k_inhibition_ATIII
    ...
    push!(kinetic_parameter_vector, 6.0*10) #11 k_FX_activation from reaction 6 in Diamond 2010 paper
    push!(kinetic_parameter_vector, 2.8E-7*10.0^6) #12 K_FX_activation
    push!(kinetic_parameter_vector, 2400.0/2) #13 k_complex
    push!(kinetic_parameter_vector, 63.5*60*500*2 )#14 k_amp_prothombinase from reaction 18 in Diamond 2010
    ...
    push!(kinetic_parameter_vector, .1) #K_amp_active_factor
    push!(kinetic_parameter_vector, .5) #k_PT_deg
    # Control parameters -
    ...
    push!(control_parameter_vector,1.0) #alpha factor_amplification_term
    push!(control_parameter_vector, 2.0) #order factor_amplification_term
   #platlet controls
	...
	push!(timing, .35) #coeff
 	...
 	@show size(kinetic_parameter_vector), size(control_parameter_vector), size(platelet_parameter_vector), size(timing)
